...
if (isset($_POST["novo_local_check"]) && $_POST["novo_local_check"] == "on") {
    ...
    $querycurrent = "INSERT INTO eventos(evento, descricao, data_fim, locais_id_local) VALUES (?, ?, ?, ?)";
    $querydatanova = "INSERT INTO eventos(evento, descricao, data_inicio, data_fim, locais_id_local) VALUES (?, ?, ?, ?, ?)";
    ...
    if ($bool == 0) {
        if (mysqli_stmt_prepare($stmt3, $query)) { // Prepare the statement
            mysqli_stmt_bind_param($stmt3, 'ssssi', $_POST["nome_evento"], $_POST["descricao_evento"], $_POST["data_inicio"], $_POST["data_fim"], $id_local_novo);
            mysqli_stmt_execute($stmt3); // Execute the prepared statement
            mysqli_stmt_close($stmt3); // Close statement
        }
    } else {
        if (mysqli_stmt_prepare($stmt3, $query)) { // Prepare the statement
            mysqli_stmt_bind_param($stmt3, 'sssi', $_POST["nome_evento"], $_POST["descricao_evento"], $_POST["data_fim"], $id_local_novo);
            mysqli_stmt_execute($stmt3); // Execute the prepared statement
            mysqli_stmt_close($stmt3); // Close statement
        }
    }
    ...
    if ($bool == 0) {
        if (mysqli_stmt_prepare($stmt, $query)) { // Prepare the statement
            mysqli_stmt_bind_param($stmt, 'ssssi', $_POST["nome_evento"], $_POST["descricao_evento"], $data_inicio, $_POST["data_fim"], $_POST["local"]);
            mysqli_stmt_execute($stmt); // Execute the prepared statement
            mysqli_stmt_close($stmt); // Close statement
        }
    } else {
        if (mysqli_stmt_prepare($stmt, $query)) { // Prepare the statement
            mysqli_stmt_bind_param($stmt, 'sssi', $_POST["nome_evento"], $_POST["descricao_evento"], $_POST["data_fim"], $_POST["local"]);
            mysqli_stmt_execute($stmt); // Execute the prepared statement
            mysqli_stmt_close($stmt); // Close statement
        }
    }
